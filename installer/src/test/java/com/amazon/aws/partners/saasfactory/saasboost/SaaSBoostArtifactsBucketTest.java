...
public class SaaSBoostArtifactsBucketTest {
        ...
        public void putFileTest() throws Exception {
                ...
                SaaSBoostArtifactsBucket testBucket = SaaSBoostArtifactsBucket.createS3ArtifactBucket(mockS3, ENV_NAME,
                                Region.US_EAST_1);
                Path localPathToTestPut = Path
                                .of(this.getClass().getClassLoader().getResource("template.yaml").toURI());
                Path exampleRemotePath = Path.of("dir", "dir2");
                testBucket.putFile(mockS3, localPathToTestPut, exampleRemotePath);
                Mockito.verify(mockS3).putObject(putObjectRequestArgumentCaptor.capture(),
                ...
                assertEquals("Put different length object to remote location. Wrong file?",
                                localPathToTestPut.toFile().length(),
                                requestBodyArgumentCaptor.getValue().contentLength());
        }

        @Test
        public void createBucketLocationConstraintTest() {
                ArgumentCaptor<CreateBucketRequest> createBucketRequestArgumentCaptor = ArgumentCaptor
                                .forClass(CreateBucketRequest.class);
                ...
                if (capturedCreateBucketRequest.createBucketConfiguration() != null) {
                        ...
                        // there is implicitly no location constraint (because constraint must be part
                        // of the config)
                        assertNull("No location constraint should be provided for buckets in us-east-1",
                                        createBucketRequestArgumentCaptor.getValue().createBucketConfiguration()
                                                        .locationConstraint());
                }
                Mockito.reset(mockS3);
        ...
        public void createBucketServerSideEncryptionTest() {
                ArgumentCaptor<PutBucketEncryptionRequest> putBucketEncryptionRequestArgumentCaptor = ArgumentCaptor
                                .forClass(PutBucketEncryptionRequest.class);
                SaaSBoostArtifactsBucket createdBucket = SaaSBoostArtifactsBucket.createS3ArtifactBucket(mockS3,
                                ENV_NAME, Region.US_EAST_1);
                Mockito.verify(mockS3).putBucketEncryption(putBucketEncryptionRequestArgumentCaptor.capture());
                PutBucketEncryptionRequest capturedPutBucketEncryptionRequest = putBucketEncryptionRequestArgumentCaptor
                                .getValue();
        ...
        public void createBucketBucketPolicyTest() {
                ArgumentCaptor<PutBucketPolicyRequest> putBucketPolicyArgumentCaptor = ArgumentCaptor
                                .forClass(PutBucketPolicyRequest.class);
                SaaSBoostArtifactsBucket createdBucket = SaaSBoostArtifactsBucket.createS3ArtifactBucket(mockS3,
                                ENV_NAME, Region.US_EAST_1);
