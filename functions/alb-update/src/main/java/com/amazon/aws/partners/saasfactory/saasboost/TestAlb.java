...
public class TestAlb {
        public static void main(String[] args) throws Exception {
                 ...
                 * .endpointOverride(new URI("https://elasticloadbalancing." + AWS_REGION.id() +
                 * ".amazonaws.com")) // will break in China regions
                ...
                try {
                        ...
                        String status = "false";
                        // arn:aws-cn:elasticloadbalancing:us-west-2:573838506705:loadbalancer/
                        ...
                        if ("saas-boost".equals(source)) {
                                DescribeLoadBalancersResponse response = elbClient.describeLoadBalancers(
                                                DescribeLoadBalancersRequest.builder().names(albNames).build());
                                LoadBalancer loadBalancer = response.loadBalancers().get(0);
                                System.out.println(("Load Balancer " + loadBalancer.toString()));

                                DescribeListenersResponse listenersResponse = elbClient
                                                .describeListeners(DescribeListenersRequest.builder()
                                ...
                                DescribeRulesResponse rulesResponse = elbClient.describeRules(DescribeRulesRequest
                                                .builder().listenerArn(listener.listenerArn()).build());
                                Action action;
                                if ("false".equalsIgnoreCase(status)) {
                                        ...
                                        DescribeTargetGroupsResponse targetGroupsResponse = elbClient
                                                        .describeTargetGroups(DescribeTargetGroupsRequest.builder()
                                                                        .names(albNames) // target group names same as
                                                                                         // ALB names in format of
                                                                                         // tenant-shortid
                                                                        .build());
                                ...
                                for (Rule rule : rulesResponse.rules()) {
                                        ...
                                        if (!rule.isDefault()) {
                                                condition = RuleCondition.builder()
                                                                ...
                                                                .values("*")
                                                                .build();
                                                elbClient.modifyRule(ModifyRuleRequest.builder().ruleArn(rule.ruleArn())
                                                                .conditions(condition).actions(action).build());
                                        } else {
